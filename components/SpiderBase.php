...
class SpiderBase extends \yii\base\Component
{
    ...
    public function addOffer($newOffer, $tagIds = [])
    {
        ...
        if (!static::getCategoryIdByOfferTitle($newOffer['title'])) {
            Yii::warning('Fail to get category ID by title: ' . $newOffer['title'] );
        }
    ...
    public function getCategoryIdByOfferTitle($title)
    {
        $matches = [
            // 奶粉牛奶
            11  => ['奶粉', '婴儿配方奶粉', '爱他美', 'Abbott', '牛奶', '美赞臣'],
            // 营养辅食
            12  => ['果汁泥', '果泥', '果蔬泥', '维生素D3', '辅食', '米糊', '维生素滴剂', '曲奇',
                '叶酸', '营养素', '童年时光', '钙尔奇', '泡芙', '补血', '补铁', '谷物棒', '棒棒糖',
                '米粉', '米饼', '挪威小鱼', 'Gerber', '维生素软糖', '海藻油', '面条', '鳕鱼'],
            // 尿裤湿巾
            13  => ['拉拉裤', '纸尿裤', '尿不湿', '成长裤', '湿巾', '柔湿巾', '尿布', '纸巾', 'MamyPoko',
                '乐玩裤', '抽纸', '柔巾', 'Merries'],
            // 洗护用品
            14  => ['香皂', '指甲钳', '指甲剪', '洗衣液', '洗衣皂', '护唇膏', '牙刷', '澡盆', '浴盆',
                '洗脸盆', '宝宝金水', '护臀霜', '护臀膏', '防蚊', '驱蚊', '清洁', '蚊香', '润肤', '护肤',
                '沐浴', '洗发', '修护膏', '防护霜', '洗澡粉', '洗涤剂', '精油', '护理套装', '衣物柔顺剂',
                '滋润膏', '洗护', '洗澡网', '脸盆', '营养霜', '暖贴宝宝贴', '爽身粉', '棉棒', '棉签', '牙膏',
                'MiYOSHi', '面霜', '洗手液'],
            // 喂养用品
            15  => ['奶嘴', '奶瓶', '退热贴','吃饭兜', '围兜', '套装食具', '咬胶', '牙胶', '脐贴', 
                '儿童杯', '磨牙棒', '吸管杯', '保温杯', '保温瓶', '新安怡', '叉勺', '学饮杯', '餐具',
                '餐盘','尿垫', '满趣健', '水壶', '水杯'],
            // 家纺服饰
            16  => ['睡袋', '行李箱', '餐椅', '防护插排', '护肚脐围', '暖手抱枕', '抱被', '包被','被子', '抱毯',
                '肚兜', '旅游朋友颈枕', '防撞条', '手帕', '方巾', '口水巾', '手绢', '乳胶枕', '罗莱'],
            // 玩具乐器
            17  => ['儿童文具', '机器人', '火车', '玩具', '爬行垫', '游戏围栏', '手表', '孩之宝', '乐高',
                '梦想屋', '邦尼兔', '费雪', '澳贝','早教机', '画板', '电子琴', '小泰克', '柔韧手抓球',
                '手办', '拼图', 'MEGA BLOKS', '钢琴', '滑步车', '3D益智', '跳舞毯', '游泳圈', 'Teddy Bear',
                '模型', '美家宝', '马头面具', '四轴飞行', '奥迪双钻', '立体手工', '风火轮', '画布', 'WowWee',
                '积木', '芭比娃娃', '海洋球', '卡特彼勒'],
            // 童车家具
            18  => ['童车', '童床', '推车', '三轮车', '伞车', '婴儿床', '马桶', '坐便器', '学习桌', '收纳箱', '摇椅',
                '储物架', '花洒'],
            // 童装童鞋
            19  => ['太阳镜', '速干裤', '篮球服', '儿童午餐包', '书包', '溜冰鞋', '学步鞋', '童鞋', '儿童圆领内衣',
                '凉鞋', '连体衣', '长裤', '羽绒服', '童袜', '袜子', '婴儿服饰', '儿童纯棉内衣套装', '外套', '秋装',
                '训练鞋', '运动鞋', '毛衣', '足球服', 'T恤' ],
            // 安全座椅
            20  => ['安全座椅', 'Britax', '汽车座椅'],
            // 妈妈用品
            21  => ['婴儿背带', '乳头霜', 'Mavala Stop', '乳垫', 'Bio-Oil', '吸乳器', '额温枪', 
                '吸奶器', '暖奶器', '腰凳', '背带', '托腹带', '母乳储存', '储奶', '护理霜', '耳温枪',
                '体温计', '文胸', 'KOSE', '母乳保鲜', '孕吐茶', '母婴包', '收紧矫正带', '哺乳内衣', '塑腰',
                '大正制药', '妊娠纹', '抱婴带', '暖宝宝贴', '温奶', '收腹', '防辐射服', '待产包', '哺乳背心',
                '哺乳枕'],
            // 图书影音
            22  => ['翻翻书', '宝宝营养餐', '百科全书', 'DK儿童'],
            // 美食生鲜
            23  => ['肉', '月子茶', '亚麻籽油', '大叶红茶', '金丝小枣', '婴幼儿纯核桃油'],
            // 家用电器
            24  => ['理发器', '雾化机', '电动牙刷', '洗鼻器套装', '调奶器', '消毒锅', '调理机'],
